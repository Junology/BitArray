...
public:
    ...
    static_assert(length > 0, "Zero length BitArray is not allowed.");
...
private:
    ...
    constexpr PopIterator(ContainerBeginTag, chunk_type const (&chunks)[length])
        noexcept
    ...
    constexpr PopIterator(ContainerEndTag, chunk_type const (&chunks)[length])
        noexcept
        : m_chunks(chunks), m_pos(length-1), m_value{}
    {}
